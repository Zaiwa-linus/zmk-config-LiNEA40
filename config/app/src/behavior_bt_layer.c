...
static int update_layers_for_profile(const struct device *dev, int profile) {
    const struct behavior_bt_layer_config *config = dev->config;

    int layer1 = config->layer_profile_1;
    int layer2 = config->layer_profile_2;
